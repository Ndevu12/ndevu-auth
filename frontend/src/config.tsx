      ...
      override: {
        functions: (originalImplementation) => ({
          ...
          getMFARequirementsForAuth: () => [
            {
              oneOf: [MultiFactorAuth.FactorIds.OTP_EMAIL],
            },
          ],
        ...
        functions: (original) => {
          return {
            ...
            getGlobalClaimValidators: (input) => {
              const emailVerificationClaimValidator =
                input.claimValidatorsAddedByOtherRecipes.find(
                  (v) => v.id === EmailVerification.EmailVerificationClaim.id
                );
              if (emailVerificationClaimValidator) {
                const filteredValidators =
                  input.claimValidatorsAddedByOtherRecipes.filter(
                    (v) => v.id !== EmailVerification.EmailVerificationClaim.id
                  );
